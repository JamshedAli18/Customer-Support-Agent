...
INTENT options:
- "inquiry": any question answerable from product knowledge — specs, setup, policies, shipping, returns, warranty, payment, how something works, recommendations ("is this good for X", "should I get Y"), or guidance requests
- "complaint": user reports a problem, defect, or dissatisfaction with the product or an order
...
def empathetic_response_node(state: dict) -> dict:
    same_issue_streak = state.get("same_issue_streak", 0)

    if same_issue_streak >= 3:
        state["escalate"] = True
        print(f"[empathetic_response_node] Same issue unresolved for {same_issue_streak} consecutive turns, flagging for escalation")
        return state
...
def warranty_claim_node(state: dict) -> dict:
    ...
    in this session already reached a terminal outcome (filed/abandoned/
    declined), starts a completely fresh claim instead of reusing stale
    details from that earlier attempt.
    """
    ...
    history_text_for_extraction = format_history_for_prompt(get_recent_history(state["messages"], limit=10))
    extraction_response = client.chat.completions.create(
